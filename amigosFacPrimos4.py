...
def amigos(tope):
    ...
    # suma_divisores[n] guardará la función sigma(n)
    # suma_potencias_menor_primo[n] guardará (1 + p + p^2 + ... + p^k) 
    # siendo p el menor factor primo de n y p^k la máxima potencia que lo divide.
    suma_divisores = [0] * (tope + 1)
    suma_potencias_menor_primo = [0] * (tope + 1)
    lista_primos = []
    
    # Optimizamos el acceso a métodos locales
    agregar_primo = lista_primos.append
    
    # Caso base para la criba
    suma_divisores[1] = 1
    ...
    for i in range(2, tope + 1):
        ...
        for primo in lista_primos:
            ...
            if i % primo == 0:
                ...
                # Propiedad multiplicativa: sigma(p * i) = sigma(p) * sigma(i)
    ...
    # Usamos un rango hasta tope + 1 para no ignorar el último valor
    for i in range(2, tope + 1):
        # s_propia: Suma de los divisores menores que el número (sigma(i) - i)
        s_propia = suma_divisores[i] - i
        
        # Filtros: i < s_propia evita duplicados y perfectos. 
        # s_propia <= tope evita errores de índice.
